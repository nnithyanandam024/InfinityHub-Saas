...
import { View, Text, StyleSheet, TextInput, TouchableOpacity, SafeAreaView, ScrollView, Image, StatusBar } from 'react-native';
...
export const BarcodeScannerScreen = ({ navigation, route }) => {
    const { products, formatPrice } = useTenant();
    const onScan = route?.params?.onScan;
    const [barcodeInput, setBarcodeInput] = useState('');
    const [matchedProduct, setMatchedProduct] = useState(products[0] || null);
    const handleSearch = (code) => {
        ...
        const found = products.find(p => (p.barcode && p.barcode.toLowerCase().includes(clean)) ||
    ...
    return (<SafeAreaView style={styles.safeArea}>
      ...
      {navigation?.canGoBack?.() ? (<View style={styles.navHeader}>
          <TouchableOpacity style={styles.backBtn} onPress={() => navigation.goBack()} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <Icon name="chevronRight" size={18} color={theme.colors.navy} style={{ transform: [{ rotate: '180deg' }] }}/>
            <Text style={styles.backText}>Back</Text>
          </TouchableOpacity>
          <Text style={styles.navHeaderTitle}>Barcode Scanner</Text>
          <View style={{ width: 48 }}/>
        </View>) : (<AppHeader navigation={navigation}/>)}
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.searchRow}>
            <TextInput style={styles.input} placeholder="Enter barcode or SKU number..." placeholderTextColor={theme.colors.muted} value={barcodeInput} onChangeText={handleSearch} keyboardType="default" autoCapitalize="none"/>
            {barcodeInput.length > 0 && (<TouchableOpacity onPress={() => {
                setBarcodeInput('');
                setMatchedProduct(null);
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.quickBarcodes}>
          {products.slice(0, 4).map(p => (<TouchableOpacity key={p.id} style={[styles.quickChip, matchedProduct?.id === p.id && styles.quickChipActive]} onPress={() => handleSearch(p.barcode || p.sku)}>
        ...
        {/* Scan Result Card */}
        {matchedProduct ? (<View style={styles.resultCard}>
            <View style={styles.resultHeader}>
              <Text style={styles.resultBadgeText}>MATCH FOUND</Text>
              <Badge label={matchedProduct.stockQuantity > 0 ? 'In Stock' : 'Depleted'} variant={matchedProduct.stockQuantity > 0 ? 'success' : 'danger'} size="sm"/>
            </View>

            <View style={styles.resultBody}>
              <View style={styles.resultThumbWrap}>
                {matchedProduct.thumbnailPath || matchedProduct.imagePath ? (<Image source={{ uri: matchedProduct.thumbnailPath || matchedProduct.imagePath }} style={styles.resultThumb} resizeMode="cover"/>) : (<View style={styles.resultFallbackThumb}>
            ...
            <View style={styles.resultActions}>
              {onScan ? (<Button label="Add to Cart" onPress={() => {
                    onScan(matchedProduct.barcode || matchedProduct.sku);
                    navigation.goBack();
                }} variant="primary" icon="cart" style={{ flex: 1 }}/>) : (<>
                  <Button label="Adjust Stock" onPress={() => navigation.navigate('StockAdjustment', { product: matchedProduct })} variant="primary" icon="stock" style={{ flex: 1 }}/>
                  <Button label="Details" onPress={() => navigation.navigate('ProductDetail', { product: matchedProduct })} variant="outline" style={{ width: 90 }}/>
                </>)}
            </View>
          </View>) : (<View style={styles.noMatchCard}>
