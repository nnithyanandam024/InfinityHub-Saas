...
import { View, Text, StyleSheet, TouchableOpacity, TextInput, ScrollView, FlatList, SafeAreaView, StatusBar, Platform, Alert } from 'react-native';
...
export const PosTerminalScreen = ({ navigation }) => {
    const { products, categories } = useTenant();
    const { cart, addToCart, updateCartItemQuantity, removeFromCart, clearCart, appliedDiscount, setAppliedDiscount, selectedCustomer, setSelectedCustomer, currentShift, openShift, closeShift, checkout, totals, taxConfig } = usePos();
    ...
    const renderProductItem = ({ item }) => {
        ...
        return (<TouchableOpacity style={[styles.productCard, inCartQty > 0 && styles.productCardInCart]} onPress={() => addToCart(item)} activeOpacity={0.7}>
        <View style={styles.cardHeader}>
          <Text style={styles.productSku}>{item.sku}</Text>
          {isLowStock ? (<Badge label="Low Stock" variant="warning" size="sm"/>) : (<Text style={styles.stockText}>{item.stockQuantity} in stock</Text>)}
        </View>
        ...
        <View style={styles.cardFooter}>
          ...
          {inCartQty > 0 ? (<View style={styles.inCartBadge}>
              <Text style={styles.inCartText}>{inCartQty} in cart</Text>
            </View>) : (<View style={styles.addBtnCircle}>
    ...
    return (<SafeAreaView style={styles.container}>
      ...
      <View style={styles.topHeader}>
        <View style={styles.headerTitleCol}>
          <View style={styles.storeRow}>
            ...
            <TouchableOpacity style={styles.shiftBadge} onPress={() => setIsShiftModalOpen(true)}>
        ...
        <TouchableOpacity style={styles.appSwitchBtn} onPress={() => setIsAppLauncherOpen(true)}>
      ...
      <View style={styles.searchBarRow}>
        <View style={styles.searchInputWrap}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search SKU, Product Name or Barcode..." placeholderTextColor={theme.colors.muted} value={searchQuery} onChangeText={setSearchQuery}/>
        ...
        <TouchableOpacity style={styles.barcodeScanBtn} onPress={handleBarcodePress} activeOpacity={0.8}>
      ...
      <View style={styles.categoriesWrapper}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.catScroll}>
          <TouchableOpacity style={[styles.catPill, selectedCategory === 'all' && styles.catPillActive]} onPress={() => setSelectedCategory('all')}>
            <Text style={[styles.catPillText, selectedCategory === 'all' && styles.catPillTextActive]}>
              All Items
            </Text>
          </TouchableOpacity>

          {categories.map(c => (<TouchableOpacity key={c.id} style={[styles.catPill, selectedCategory === c.id && styles.catPillActive]} onPress={() => setSelectedCategory(c.id)}>
      ...
      {/* Product Grid */}
      <FlatList data={filteredProducts} keyExtractor={item => item.id} renderItem={renderProductItem} numColumns={2} columnWrapperStyle={styles.gridRow} contentContainerStyle={styles.gridContent} showsVerticalScrollIndicator={false}/>

      {/* Floating Bottom Cart Bar */}
      {cart.length > 0 && (<View style={styles.cartStickyBar}>
          <TouchableOpacity style={styles.cartInfoTouch} onPress={() => setIsCartOpen(true)} activeOpacity={0.85}>
          ...
          <TouchableOpacity style={styles.checkoutQuickBtn} onPress={() => setIsPaymentModalOpen(true)} activeOpacity={0.85}>
        ...
        <View style={styles.cartModalBackdrop}>
          ...
          <View style={styles.cartSheet}>
            ...
            <View style={styles.ticketActionsRow}>
              <TouchableOpacity style={[styles.ticketActionChip, appliedDiscount && styles.ticketActionChipActive]} onPress={() => setIsDiscountModalOpen(true)}>
            ...
            : 'Add Discount'}
              ...
              <TouchableOpacity style={[styles.ticketActionChip, selectedCustomer && styles.ticketActionChipActive]} onPress={() => {
            ...
            <ScrollView style={styles.cartItemsScroll} showsVerticalScrollIndicator={false}>
              {cart.map(item => (<View key={item.productId} style={styles.cartItemRow}>
                  ...
                  <View style={styles.cartItemStepper}>
                    <TouchableOpacity style={styles.cartStepBtn} onPress={() => updateCartItemQuantity(item.productId, item.quantity - 1)}>
            ...
            <View style={styles.summaryBreakdown}>
              ...
              {appliedDiscount && (<View style={styles.breakdownRow}>
            ...
            {/* Sheet Footer Action */}
            <Button label={`Proceed to Payment (₹${totals.grandTotal.toFixed(2)})`} variant="primary" onPress={() => {
            setIsCartOpen(false);
            setIsPaymentModalOpen(true);
        }}/>
          </View>
      ...
      {/* Discount Modal */}
      <MobileDiscountModal visible={isDiscountModalOpen} onClose={() => setIsDiscountModalOpen(false)} subtotal={totals.subtotal} currentDiscount={appliedDiscount} onApplyDiscount={setAppliedDiscount}/>

      {/* Payment Modal */}
      <MobilePaymentModal visible={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} onCompleteCheckout={handleCheckoutComplete}/>

      {/* Shift Modal */}
      <MobileShiftModal visible={isShiftModalOpen} onClose={() => setIsShiftModalOpen(false)} mode={currentShift ? 'close' : 'open'} currentShift={currentShift} onOpenShift={openShift} onCloseShift={closeShift}/>
...
//# sourceMappingURL=PosTerminalScreen.js.map