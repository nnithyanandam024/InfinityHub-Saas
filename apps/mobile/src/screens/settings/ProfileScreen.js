      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.userCard}>
          ...
          <View style={{ flex: 1, marginLeft: 14 }}>
            ...
            <View style={{ marginTop: 6 }}>
              <Badge label={getRoleLabel(user?.role)} variant={user?.role === 'SUPER_ADMIN' ? 'danger' : user?.role === 'TENANT_OWNER' ? 'primary' : 'muted'} size="sm"/>
            </View>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoVal}>{tenant.category || 'Mobile & Electronics Retail'}</Text>
          </View>
          <View style={styles.infoRow}>
        ...
        <View style={styles.personaRow}>
          {DEMO_PERSONAS.slice(0, 3).map(p => (<TouchableOpacity key={p.key} style={[styles.personaChip, user?.id === p.key && styles.personaChipActive]} onPress={() => loginAsPersona(p.key)} activeOpacity={0.75}>
        ...
        {/* Sign Out Action */}
        <Button label="Sign Out" onPress={handleLogout} variant="danger" style={{ marginTop: 24 }}/>
      </ScrollView>
    </SafeAreaView>);
};
