...
export const DashboardScreen = ({ navigation }) => {
    ...
    return (<SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh} colors={[theme.colors.primary]}/>}>
        ...
        <View style={styles.kpiGrid}>
          {/* Total SKUs */}
          <TouchableOpacity style={[styles.kpiCard, styles.kpiBlue]} onPress={() => navigation.navigate('InventoryTab', { screen: 'ProductList' })} activeOpacity={0.75}>
          ...
          {/* Low Buffer Warning */}
          <TouchableOpacity style={[styles.kpiCard, styles.kpiAmber]} onPress={() => navigation.navigate('StockTab', { screen: 'StockOverview', params: { filter: 'low' } })} activeOpacity={0.75}>
            <View style={styles.kpiHeader}>
          ...
          {/* Depleted Stock */}
          <TouchableOpacity style={[styles.kpiCard, styles.kpiRose]} onPress={() => navigation.navigate('StockTab', { screen: 'StockOverview', params: { filter: 'out' } })} activeOpacity={0.75}>
            <View style={styles.kpiHeader}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.actionRow}>
            {isEditor && (<TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('InventoryTab', { screen: 'NewProduct' })} activeOpacity={0.75}>
            ...
            <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('Scanner')} activeOpacity={0.75}>
            ...
            <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('StockTab')} activeOpacity={0.75}>
        ...
        <View style={styles.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.carouselContainer}>
            ...
            return (<TouchableOpacity key={prod.id} style={styles.carouselCard} onPress={() => openQuickSheet(prod)} activeOpacity={0.8}>
                  <View style={styles.carouselThumbWrap}>
                    {imgUri ? (<Image source={{ uri: imgUri }} style={styles.carouselThumb} resizeMode="cover"/>) : (<View style={styles.carouselFallback}>
                        <Text style={styles.carouselFallbackText}>{prod.name.slice(0, 2).toUpperCase()}</Text>
                      </View>)}
                    <View style={styles.carouselBadgeWrap}>
                      <Badge label={isOut ? 'Depleted' : isLow ? 'Low' : `${prod.stockQuantity} pcs`} variant={isOut ? 'danger' : isLow ? 'warning' : 'success'} size="sm"/>
                    </View>
                  </View>
        ...
        <View style={styles.section}>
          ...
          {lowStock.concat(outOfStock).length === 0 ? (<View style={styles.emptyCard}>
            ...
            </View>) : (lowStock.concat(outOfStock).slice(0, 3).map(item => {
            const imgUri = item.thumbnailPath || item.imagePath;
            const isOut = item.stockQuantity <= 0;
            return (<View key={item.id} style={styles.alertCard}>
                  <View style={styles.alertThumbWrap}>
                    {imgUri ? (<Image source={{ uri: imgUri }} style={styles.alertThumb} resizeMode="cover"/>) : (<View style={styles.fallbackThumb}>
                  ...
                  <TouchableOpacity style={styles.adjustSmallBtn} onPress={() => openQuickSheet(item)} activeOpacity={0.75}>
          ...
          <View style={styles.ledgerCard}>
            ...
            return (<View key={mov.id} style={[
                  ...
                  <View style={[styles.movementIconPill, { backgroundColor: isPos ? '#ECFDF5' : '#EFF6FF' }]}>
                    <Icon name={isPos ? 'plus' : 'minus'} size={12} color={isPos ? theme.colors.success : theme.colors.primary}/>
                  </View>
