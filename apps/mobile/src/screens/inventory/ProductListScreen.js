...
import { View, Text, StyleSheet, FlatList, TouchableOpacity, SafeAreaView, Image, ScrollView, StatusBar, RefreshControl } from 'react-native';
...
export const ProductListScreen = ({ navigation }) => {
    ...
    const handleInlineStep = (product, delta, e) => {
        e.stopPropagation();
        const newStock = Math.max(0, product.stockQuantity + delta);
        updateProductStock(product.id, newStock, delta > 0 ? 'Quick +1 increment' : 'Quick -1 decrement');
    };
    // Render Status Badge
    const renderStockBadge = (stock, min) => {
        if (stock <= 0)
    ...
    const renderListItem = ({ item }) => {
        ...
        return (<TouchableOpacity style={styles.listCard} activeOpacity={0.82} onPress={() => navigation.navigate('ProductDetail', { product: item })}>
        {/* Product Thumbnail with subtle corner badge */}
        <View style={styles.listThumbWrap}>
          {imgUri ? (<Image source={{ uri: imgUri }} style={styles.listThumb} resizeMode="cover"/>) : (<View style={styles.listFallbackThumb}>
              <Text style={styles.listFallbackText}>{item.name.slice(0, 2).toUpperCase()}</Text>
            </View>)}
          {isOut && (<View style={styles.thumbAlertDot}/>)}
        </View>
        ...
        <View style={styles.listRight}>
          <View style={styles.inlineStepper}>
            <TouchableOpacity style={[styles.stepperBtn, item.stockQuantity <= 0 && styles.stepperBtnDisabled]} onPress={e => handleInlineStep(item, -1, e)} disabled={item.stockQuantity <= 0} activeOpacity={0.7} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
              <Icon name="minus" size={12} color={item.stockQuantity <= 0 ? theme.colors.muted : theme.colors.navy}/>
            </TouchableOpacity>

            <TouchableOpacity style={styles.stepperCountWrap} onPress={() => handleOpenSheet(item)} activeOpacity={0.7}>
              <Text style={styles.stepperCountText}>{item.stockQuantity}</Text>
              <Text style={styles.stepperUnitText}>{item.unit}</Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.stepperBtn} onPress={e => handleInlineStep(item, +1, e)} activeOpacity={0.7} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
              <Icon name="plus" size={12} color={theme.colors.navy}/>
            </TouchableOpacity>
          </View>

          <TouchableOpacity style={styles.quickSheetTrigger} onPress={() => handleOpenSheet(item)} activeOpacity={0.7}>
    ...
    const renderGridItem = ({ item }) => {
        ...
        const isOut = item.stockQuantity <= 0;
        const isLow = !isOut && item.stockQuantity <= item.minimumStock;
        return (<TouchableOpacity style={styles.gridCard} activeOpacity={0.85} onPress={() => navigation.navigate('ProductDetail', { product: item })}>
        <View style={styles.gridImageWrap}>
          {imgUri ? (<Image source={{ uri: imgUri }} style={styles.gridImage} resizeMode="cover"/>) : (<View style={styles.gridFallback}>
              <Text style={styles.gridFallbackText}>{item.name.slice(0, 2).toUpperCase()}</Text>
            </View>)}

          <View style={styles.floatingBadgeWrap}>
            <Badge label={isOut ? 'Out of Stock' : isLow ? 'Low Stock' : `${item.stockQuantity} in stock`} variant={isOut ? 'danger' : isLow ? 'warning' : 'success'} size="sm"/>
          </View>
        </View>
        ...
        <View style={styles.gridBody}>
          ...
          <View style={styles.gridStepperRow}>
            <View style={styles.gridInlineStepper}>
              <TouchableOpacity style={[styles.gridStepBtn, item.stockQuantity <= 0 && styles.stepperBtnDisabled]} onPress={e => handleInlineStep(item, -1, e)} disabled={item.stockQuantity <= 0} activeOpacity={0.7}>
                <Icon name="minus" size={12} color={item.stockQuantity <= 0 ? theme.colors.muted : theme.colors.navy}/>
              </TouchableOpacity>

              <TouchableOpacity style={styles.gridCountBtn} onPress={() => handleOpenSheet(item)}>
                <Text style={styles.gridCountText}>{item.stockQuantity}</Text>
              </TouchableOpacity>

              <TouchableOpacity style={styles.gridStepBtn} onPress={e => handleInlineStep(item, +1, e)} activeOpacity={0.7}>
                <Icon name="plus" size={12} color={theme.colors.navy}/>
              </TouchableOpacity>
            </View>

            <TouchableOpacity style={styles.gridAdjustPill} onPress={() => handleOpenSheet(item)}>
    ...
    return (<SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.topToolbar}>
        <View style={styles.searchWrap}>
          <SearchInput value={search} onChangeText={setSearch} placeholder="Search products, SKU, barcode..." onScanPress={() => navigation.navigate('Scanner')}/>
        </View>

        {/* List / Grid Segmented Toggle */}
        <View style={styles.viewToggleGroup}>
          <TouchableOpacity style={[styles.toggleBtn, viewMode === 'list' && styles.toggleBtnActive]} onPress={() => setViewMode('list')} activeOpacity={0.7}>
            <Icon name="list" size={15} color={viewMode === 'list' ? theme.colors.primary : theme.colors.muted}/>
          </TouchableOpacity>

          <TouchableOpacity style={[styles.toggleBtn, viewMode === 'grid' && styles.toggleBtnActive]} onPress={() => setViewMode('grid')} activeOpacity={0.7}>
            <Icon name="grid" size={15} color={viewMode === 'grid' ? theme.colors.primary : theme.colors.muted}/>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <View style={styles.categoryBar}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryScroll}>
          {/* All Filter Chip */}
          <TouchableOpacity style={[styles.chip, activeFilter === 'ALL' && styles.chipActive]} onPress={() => setActiveFilter('ALL')} activeOpacity={0.8}>
        ...
        ]} onPress={() => setActiveFilter('LOW')} activeOpacity={0.8}>
        ...
        ]} onPress={() => setActiveFilter('DEPLETED')} activeOpacity={0.8}>
          ...
          {categories.map(c => {
            ...
            return (<TouchableOpacity key={c.id} style={[styles.chip, isSelected && styles.chipActive]} onPress={() => setActiveFilter(c.id)} activeOpacity={0.8}>
      ...
      {/* Product List / Grid */}
      <FlatList key={viewMode} data={filteredProducts} keyExtractor={item => item.id} numColumns={viewMode === 'grid' ? 2 : 1} columnWrapperStyle={viewMode === 'grid' ? styles.gridRow : undefined} contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={handleRefresh} colors={[theme.colors.primary]}/>} renderItem={viewMode === 'grid' ? renderGridItem : renderListItem} ListEmptyComponent={<View style={styles.emptyContainer}>
            ...
            <TouchableOpacity style={styles.resetFilterBtn} onPress={() => {
                setSearch('');
                setActiveFilter('ALL');
            }}>
      ...
      {isEditor && (<TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('NewProduct')} activeOpacity={0.85}>
          <Icon name="plus" size={18} color="#FFFFFF"/>
          <Text style={styles.fabText}>Add Product</Text>
        </TouchableOpacity>)}

      {/* Zomato-Style Quick Stock Bottom Sheet */}
      <QuickStockBottomSheet visible={isSheetVisible} product={sheetProduct} onClose={() => setIsSheetVisible(false)}/>
    </SafeAreaView>);
};
