...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, SafeAreaView, Image, StatusBar, Alert, Modal, TextInput, TouchableWithoutFeedback, Platform } from 'react-native';
...
export const ProductDetailScreen = ({ route, navigation }) => {
    ...
    const marginPct = currentProduct.sellingPrice > 0 && currentProduct.costPrice
    ...
    const handleDeleteProduct = () => {
        Alert.alert('Delete Product', `Are you sure you want to permanently delete "${currentProduct.name}"? This action cannot be undone.`, [
    ...
    return (<SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.header}>
        ...
        {canEdit ? (<TouchableOpacity style={styles.editHeaderBtn} onPress={handleOpenEdit} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.heroContainer}>
          {imgUri ? (<Image source={{ uri: imgUri }} style={styles.heroImage} resizeMode="cover"/>) : (<View style={styles.heroFallback}>
          ...
          <View style={styles.heroFloatingBadge}>
            <Badge label={isOut ? 'Depleted' : isLow ? 'Low Stock' : 'In Stock'} variant={isOut ? 'danger' : isLow ? 'warning' : 'success'}/>
          </View>
        </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.pricingGrid}>
            ...
            {canSeeCosts ? (<View style={styles.priceBox}>
              ...
              </View>) : (<View style={[styles.priceBox, styles.restrictedBox]}>
        ...
        <View style={styles.card}>
          ...
          {canAdjust && (<Button label="Adjust Stock Quantity" onPress={() => navigation.navigate('StockAdjustment', { product: currentProduct })} variant="primary" icon="stock" style={{ marginBottom: 10 }}/>)}

          {canEdit && (<>
              <Button label="Edit Details & Pricing" onPress={handleOpenEdit} variant="outline" style={{ marginBottom: 10 }}/>
              <Button label="Delete Product" onPress={handleDeleteProduct} variant="danger"/>
            </>)}
        </View>
      </ScrollView>
            ...
            <TouchableWithoutFeedback onPress={() => { }}>
              <View style={styles.modalSheetContainer}>
                ...
                <View style={styles.modalHeader}>
                  ...
                  <TouchableOpacity style={styles.modalCloseBtn} onPress={() => setIsEditModalVisible(false)}>
                ...
                <ScrollView contentContainerStyle={styles.modalScrollContent} showsVerticalScrollIndicator={false}>
                  <View style={styles.inputGroup}>
                    ...
                    <TextInput style={styles.textInput} value={editName} onChangeText={setEditName} placeholder="Product Name" placeholderTextColor={theme.colors.muted}/>
                  </View>

                  <View style={styles.inputRow}>
                    <View style={[styles.inputGroup, { flex: 1, marginRight: 8 }]}>
                      <Text style={styles.inputLabel}>Selling Price ({currencySymbol})</Text>
                      <TextInput style={styles.textInput} value={editSellingPrice} onChangeText={setEditSellingPrice} keyboardType="numeric" placeholder="0.00" placeholderTextColor={theme.colors.muted}/>
                    </View>

                    {canSeeCosts && (<View style={[styles.inputGroup, { flex: 1, marginLeft: 8 }]}>
                        <Text style={styles.inputLabel}>Cost Price ({currencySymbol})</Text>
                        <TextInput style={styles.textInput} value={editCostPrice} onChangeText={setEditCostPrice} keyboardType="numeric" placeholder="0.00" placeholderTextColor={theme.colors.muted}/>
                      </View>)}
                  </View>
                  ...
                  <View style={styles.inputRow}>
                    <View style={[styles.inputGroup, { flex: 1, marginRight: 8 }]}>
                      ...
                      <TextInput style={styles.textInput} value={editMinStock} onChangeText={setEditMinStock} keyboardType="numeric" placeholder="10" placeholderTextColor={theme.colors.muted}/>
                    </View>

                    <View style={[styles.inputGroup, { flex: 1, marginLeft: 8 }]}>
                      <Text style={styles.inputLabel}>Reorder Quantity</Text>
                      <TextInput style={styles.textInput} value={editReorderQty} onChangeText={setEditReorderQty} keyboardType="numeric" placeholder="25" placeholderTextColor={theme.colors.muted}/>
                    </View>
                  </View>

                  <View style={styles.modalActionButtons}>
                    <Button label="Cancel" onPress={() => setIsEditModalVisible(false)} variant="outline" style={{ flex: 1, marginRight: 8 }}/>
                    <Button label="Save Changes" onPress={handleSaveEdit} variant="primary" style={{ flex: 1, marginLeft: 8 }}/>
                  </View>
                </ScrollView>
              </View>
