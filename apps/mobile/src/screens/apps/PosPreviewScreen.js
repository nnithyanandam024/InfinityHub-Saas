...
export const PosPreviewScreen = ({ navigation }) => {
    const { products, tenant, formatPrice } = useTenant();
    const { switchApp } = useApp();
    const [cart, setCart] = useState(products.slice(0, 2).map(p => ({ product: p, quantity: 1 })));
    ...
    const handleCheckout = () => {
        ...
        Alert.alert('Transaction Complete', `Payment of ${formatPrice(total)} processed successfully for ${tenant.name}. Receipt #INV-${Math.floor(1000 + Math.random() * 9000)} generated.`, [
            {
                text: 'New Sale',
                onPress: () => setCart([])
            }
    ...
    return (<SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.itemPickerRow}>
          {products.slice(0, 6).map(p => (<TouchableOpacity key={p.id} style={styles.itemPill} onPress={() => addItem(p)} activeOpacity={0.75}>
        ...
        <View style={styles.cartCard}>
          {cart.length === 0 ? (<View style={styles.emptyCart}>
              <Text style={styles.emptyCartText}>No items added to current bill</Text>
            </View>) : (cart.map(item => (<View key={item.product.id} style={styles.cartRow}>
                <View style={{ flex: 1 }}>
                ...
                <View style={styles.qtyControl}>
                  <TouchableOpacity style={styles.qtyBtn} onPress={() => removeItem(item.product.id)}>
                    <Text style={styles.qtyBtnText}>-</Text>
                  </TouchableOpacity>
                  <Text style={styles.qtyText}>{item.quantity}</Text>
                  <TouchableOpacity style={styles.qtyBtn} onPress={() => addItem(item.product)}>
                    <Text style={styles.qtyBtnText}>+</Text>
                  </TouchableOpacity>
                </View>
          ...
          {/* Bill Summary */}
          {cart.length > 0 && (<View style={styles.billSummary}>
        ...
        <Button label={`Charge ${formatPrice(total)}`} onPress={handleCheckout} variant="primary" size="lg" style={{ marginTop: 16 }}/>
      </ScrollView>
    </SafeAreaView>);
};
...
//# sourceMappingURL=PosPreviewScreen.js.map