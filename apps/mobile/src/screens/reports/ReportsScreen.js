    ...
    return (<SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.categoryCard}>
          {categoryStats.map((cat, index) => (<View key={cat.id} style={[
                styles.categoryRow,
        ...
        {urgentReorders.length > 0 && (<>
            <View style={styles.sectionHeaderRow}>
              ...
              <Badge label={`${urgentReorders.length} ITEMS`} variant="warning" size="sm"/>
            </View>
            <View style={styles.reorderCard}>
              {urgentReorders.map((prod, index) => {
                const isOut = prod.stockQuantity <= 0;
                return (<View key={prod.id} style={[
                        styles.reorderItem,
                    ...
                    <View style={{ flex: 1, marginRight: 10 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                        ...
                        <Badge label={isOut ? 'DEPLETED' : 'LOW'} variant={isOut ? 'danger' : 'warning'} size="sm"/>
                      </View>
                    ...
                    <Button label="Reorder" onPress={() => navigation.navigate('StockAdjustment', { product: prod })} variant="outline" style={styles.reorderBtn}/>
                  </View>);
            })}
            </View>
          </>)}
