...
export const MobileSalesReturnModal = ({ visible, onClose, invoice, onSuccess }) => {
    ...
    if (!invoice)
        return null;
    const handleQtyChange = (productId, delta, maxQty) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Alert.alert('Sales Return Processed', `Credit note ${result.creditNoteNumber} issued for ₹${totalRefund.toFixed(2)}. Inventory stock has been restored.`);
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.backdrop}>
        ...
        <View style={styles.sheet}>
          ...
          <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
            ...
            const qty = returnQtys[item.productId] || 0;
            return (<View key={item.productId} style={styles.itemRow}>
                    ...
                    <View style={styles.stepper}>
                      <TouchableOpacity style={styles.stepBtn} onPress={() => handleQtyChange(item.productId, -1, item.quantity)}>
            ...
            <View style={styles.toggleRow}>
              {['cash', 'upi', 'store_credit'].map(m => (<TouchableOpacity key={m} style={[styles.toggleBtn, refundMethod === m && styles.toggleBtnActive]} onPress={() => setRefundMethod(m)}>
            ...
            <View style={styles.reasonRow}>
              {['Defective / Damaged', 'Wrong Item', 'Customer Choice', 'Expired'].map(r => (<TouchableOpacity key={r} style={[styles.chip, reason === r && styles.chipActive]} onPress={() => setReason(r)}>
          ...
          <View style={styles.footerCard}>
            ...
            <Button label={isSubmitting ? 'Issuing Credit Note...' : `Issue Return & Refund ₹${totalRefund.toFixed(2)}`} variant="danger" onPress={handleSubmit} disabled={isSubmitting || totalRefund <= 0}/>
          </View>
        </View>
...
//# sourceMappingURL=MobileSalesReturnModal.js.map