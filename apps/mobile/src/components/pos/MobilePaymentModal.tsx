...
import { PosPaymentMethod, PosPaymentRecord, PosCustomer } from '@infinityhub/types';
...
export const MobilePaymentModal: React.FC<MobilePaymentModalProps> = ({
  ...
  const [tenderedInput, setTenderedInput] = useState<string>(String(grandTotal));
  ...
  const handleCashDenom = (denom: number | 'exact') => {
    if (denom === 'exact') {
      setTenderedInput(String(grandTotal));
    } else {
    ...
    if (selectedMethod === 'credit_khata') {
      ...
      if (newBalance > selectedCustomer.creditLimit) {
        Alert.alert(
          ...
          `${selectedCustomer.name}'s credit limit is ₹${selectedCustomer.creditLimit}. Current debt is ₹${selectedCustomer.currentBalance}. This sale would exceed the limit by ₹${newBalance - selectedCustomer.creditLimit}.`
        );
        return;
      }
    }
  ...
  const qrUrl = generateUpiQrImageUrl(
    taxConfig.upiId,
    taxConfig.tradeName,
    grandTotal,
      ...
      >
        ...
        <View style={styles.sheet}>
          ...
          <View style={styles.methodsGrid}>
            ...
            <TouchableOpacity
              style={[styles.methodCard, selectedMethod === 'credit_khata' && styles.methodCardActive]}
          ...
          <ScrollView style={styles.methodContent} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.methodPane}>
                ...
                <View style={styles.denomRow}>
                  ...
                  {[100, 200, 500, 2000].map(d => (
                    <TouchableOpacity key={d} style={styles.denomBtn} onPress={() => handleCashDenom(d)}>
            ...
            {/* UPI QR PAYMENT UI */}
            {selectedMethod === 'upi' && (
              <View style={[styles.methodPane, { alignItems: 'center' }]}>
                ...
                <View style={styles.inputContainer}>
                  <TextInput
                    ...
                    placeholder="Optional: Enter UPI UTR / Reference ID"
                    value={referenceId}
            ...
            {selectedMethod === 'card' && (
              <View style={styles.methodPane}>
                ...
                <View style={styles.inputContainer}>
                  <TextInput
                    style={styles.textInput}
                    placeholder="Enter approval auth code (e.g. 849201)"
                    value={referenceId}
                    onChangeText={setReferenceId}
                  />
                </View>
          ...
          <View style={styles.footer}>
            <Button
              label={isProcessing ? 'Processing Order...' : `Complete Order & Charge ₹${grandTotal.toFixed(2)}`}
              variant="primary"
