...
export const MobileDrawerMovementModal = ({ visible, onClose, onRecordMovement }) => {
    const [movementType, setMovementType] = useState('cash_in');
    const [amountInput, setAmountInput] = useState('');
    const [reason, setReason] = useState('');
    const [isSubmitting, setIsSubmitting] = useState(false);
    const amount = parseFloat(amountInput) || 0;
    const handleReasonPreset = (preset) => {
        setReason(preset);
    };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.backdrop}>
        ...
        <View style={styles.sheet}>
          ...
          <View style={styles.toggleRow}>
            <TouchableOpacity style={[styles.toggleBtn, movementType === 'cash_in' && styles.toggleBtnActive]} onPress={() => setMovementType('cash_in')}>
              <Text style={[styles.toggleText, movementType === 'cash_in' && styles.toggleTextActive]}>
                + Cash In (Paid-In)
              </Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.toggleBtn, movementType === 'cash_out' && styles.toggleBtnActive]} onPress={() => setMovementType('cash_out')}>
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput style={styles.input} keyboardType="numeric" placeholder="e.g. 500" placeholderTextColor={theme.colors.muted} value={amountInput} onChangeText={setAmountInput} autoFocus/>
          </View>
          ...
          <View style={styles.presetGrid}>
            ...
            : ['Petty Cash Expense', 'Safe Drop / Bank Deposit', 'Vendor Cash Advance', 'Tea & Snacks']).map(p => (<TouchableOpacity key={p} style={[styles.presetChip, reason === p && styles.presetChipActive]} onPress={() => handleReasonPreset(p)}>
          ...
          <View style={[styles.inputContainer, { height: 48, marginTop: 4 }]}>
            <TextInput style={styles.input} placeholder="Or type custom reason..." placeholderTextColor={theme.colors.muted} value={reason} onChangeText={setReason}/>
          </View>

          <View style={styles.actions}>
            <Button label={isSubmitting ? 'Recording...' : `Confirm ${movementType === 'cash_in' ? 'Cash In' : 'Cash Out'}`} variant={movementType === 'cash_in' ? 'primary' : 'danger'} onPress={handleSubmit} disabled={isSubmitting}/>
          </View>
        </View>
...
//# sourceMappingURL=MobileDrawerMovementModal.js.map