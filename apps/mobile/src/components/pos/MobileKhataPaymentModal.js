...
export const MobileKhataPaymentModal = ({ visible, onClose, customer, onRecordPayment }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Alert.alert('Payment Recorded', `Successfully received ₹${payAmount} from ${customer.name}. Remaining Khata balance is ₹${remainingDebt}.`);
            onClose();
        }
        catch (err) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.backdrop}>
        ...
        <View style={styles.sheet}>
          ...
          <View style={styles.toggleRow}>
            {['cash', 'upi', 'bank'].map(m => (<TouchableOpacity key={m} style={[styles.toggleBtn, paymentMethod === m && styles.toggleBtnActive]} onPress={() => setPaymentMethod(m)}>
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput style={styles.input} keyboardType="numeric" placeholder="e.g. 1000" placeholderTextColor={theme.colors.muted} value={amountInput} onChangeText={setAmountInput}/>
          </View>
          ...
          <View style={[styles.inputContainer, { height: 48, marginTop: 4 }]}>
            <TextInput style={styles.input} placeholder="Optional: UPI UTR or Bank Receipt No." placeholderTextColor={theme.colors.muted} value={reference} onChangeText={setReference}/>
          </View>

          <View style={styles.actions}>
            <Button label={isSubmitting ? 'Recording Payment...' : `Record Payment of ₹${payAmount.toFixed(2)}`} variant="primary" onPress={handleSubmit} disabled={isSubmitting || payAmount <= 0}/>
          </View>
        </View>
...
//# sourceMappingURL=MobileKhataPaymentModal.js.map