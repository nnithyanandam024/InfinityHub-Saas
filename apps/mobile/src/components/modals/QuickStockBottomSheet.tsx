...
export const QuickStockBottomSheet: React.FC<QuickStockBottomSheetProps> = ({
  visible,
  product,
  onClose,
  onSuccess
  ...
  const handleApplyDelta = (d: number) => {
    setQuantity(prev => Math.max(0, prev + d));
  };

  const handleSave = () => {
    const reasonLabel =
      MOVEMENT_REASONS.find(r => r.id === selectedReason)?.label || 'Quick adjustment';
            ...
            <View style={styles.sheetContainer}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.productHeader}>
                  ...
                  <View style={styles.productInfo}>
                    ...
                    <View style={styles.stockBadgeRow}>
                      <Badge
                        label={`Current: ${currentStock} ${product.unit}`}
                        variant={currentStock <= 0 ? 'danger' : currentStock <= product.minimumStock ? 'warning' : 'success'}
                        size="sm"
                      />
                    </View>
                  </View>
                ...
                <View style={styles.stepperContainer}>
                  ...
                  {delta !== 0 && (
                    <View style={[styles.deltaPill, delta > 0 ? styles.deltaPillPositive : styles.deltaPillNegative]}>
                ...
                <View style={styles.quickChipsRow}>
                  {QUICK_ADD_DELTAS.map(d => (
                    <TouchableOpacity
                      key={d}
                      style={styles.chipBtn}
                      onPress={() => handleApplyDelta(d)}
                      activeOpacity={0.75}
                    >
                ...
                <View style={styles.noteContainer}>
                  <TextInput
                    ...
                    placeholder="Optional note / reference number..."
                ...
                <View style={styles.actions}>
                  <Button
                    label={`Confirm (${quantity} ${product.unit})`}
                    onPress={handleSave}
                    variant="primary"
                    icon="check"
                  />
