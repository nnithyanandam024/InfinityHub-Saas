...
export const AppProvider = ({ children }) => {
    ...
    const [activeAppId, setActiveAppId] = useState(() => (isPosLicensed ? 'pos' : 'inventory'));
    useEffect(() => {
    ...
    const switchApp = (appId) => {
        if (appId === 'pos' && !isPosLicensed) {
            Alert.alert('License Required', `Your active workspace (${tenant?.name || 'Store'}) is licensed for Inventory Management. Billing & POS counter is not included in this plan.`);
            return;
        }
