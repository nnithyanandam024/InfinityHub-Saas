...
export const PosProvider = ({ children }) => {
    ...
    // Open Shift
    const openShift = async (startingFloat, notes) => {
        try {
    ...
    const recordDrawerMovement = async (type, amount, reason) => {
        try {
    ...
    const closeShift = async (actualCashCounted, notes) => {
        try {
    ...
    const checkout = async (paymentData) => {
        ...
        try {
            ...
            const upiQrString = generateUpiIntentUri(taxConfig.upiId, taxConfig.tradeName, totals.grandTotal, invoiceNumber);
            const newOrder = {
                id: orderId,
    ...
    // Sales Return
    const processReturn = async (invoiceId, payload) => {
        return apiClient.pos.processReturn(tenantId, invoiceId, payload);
    };
    // Record Customer Repayment
    const recordCustomerPayment = async (customerId, amount, paymentMethod = 'cash', reference) => {
        try {
...
//# sourceMappingURL=PosContext.js.map