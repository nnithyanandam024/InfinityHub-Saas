...
function numberToIndianWords(num) {
    ...
    if (crore > 0)
        str += formatSegment(crore) + ' Crore ';
    if (lakh > 0)
        str += formatSegment(lakh) + ' Lakh ';
    if (thousand > 0)
        str += formatSegment(thousand) + ' Thousand ';
    if (remainder > 0)
        str += formatSegment(remainder) + ' ';
    return (str.trim() + ' Rupees Only');
}
function initTenantPosData(store) {
    if (!store.posOrders)
        store.posOrders = [];
    if (!store.invoices)
        store.invoices = [];
    if (!store.shifts)
        store.shifts = [];
    if (!store.cashMovements)
        store.cashMovements = [];
    if (!store.posCustomers)
        store.posCustomers = [];
    if (!store.creditTransactions)
        store.creditTransactions = [];
    if (!store.posAuditLogs)
        store.posAuditLogs = [];
    if (!store.posReturns)
        store.posReturns = [];
...
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        if (pathname === '/api/v1/sync/poll' && req.method === 'GET') {
            ...
            const filtered = recentEvents.filter(e => (!tenantId || e.tenantId === tenantId) && (!since || e.timestamp > since));
        ...
        if (pathname === '/api/v1/pos/shifts/movement' && req.method === 'POST') {
            ...
            if (!shift)
                return sendError(res, 400, 'No active open shift found');
        ...
        if (pathname === '/api/v1/pos/shifts/close' && req.method === 'POST') {
            ...
            const shift = store.shifts?.find(s => s.status === 'open');
            if (!shift)
                return sendError(res, 400, 'No active open shift found to close');
            ...
            if (body.notes)
                shift.notes = (shift.notes ? shift.notes + ' | ' : '') + body.notes;
            store.posAuditLogs?.unshift({
                id: `audit-${Date.now()}`,
                tenantId,
                cashierId: shift.cashierId,
        ...
        if (pathname === '/api/v1/pos/checkout' && req.method === 'POST') {
            ...
            const payments = body.payments && body.payments.length > 0 ? body.payments : [
            ...
            for (const p of payments) {
                if (p.method === 'cash')
                    cashTendered += Number(p.amount) || 0;
                else if (p.method === 'upi')
                    upiTendered += Number(p.amount) || 0;
                else if (p.method === 'card')
                    cardTendered += Number(p.amount) || 0;
                else if (p.method === 'credit_khata')
                    creditKhataTendered += Number(p.amount) || 0;
            }
        ...
        if (pathname.startsWith('/api/v1/pos/invoices/') && pathname.endsWith('/return') && req.method === 'POST') {
            ...
            const inv = store.invoices?.find(i => i.id === invoiceId || i.invoiceNumber === invoiceId);
            if (!inv)
                return sendError(res, 404, 'Original invoice not found');
            const returnedItemsPayload = body.returnedItems || body.items || [];
