...
export class ApkBuilderService {
  ...
  static getApkFilePath(tenantId: string): string | null {
    ...
    if (!fs.existsSync(tenantBuildDir)) return null;
  ...
  static async buildBrandedApk(
    ...
    fs.writeFileSync(
      path.join(tenantBuildDir, 'tenant_config.json'),
    ...
    const updatedBranding: TenantMobileBranding = {
      appName,
      shortName,
      logoUrl,
      primaryColor,
