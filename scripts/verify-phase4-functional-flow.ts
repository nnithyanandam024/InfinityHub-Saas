...
async function runSuite() {
  ...
  assert(
    initialMovements.length === 1 &&
    initialMovements[0].type === 'correction' &&
    initialMovements[0].quantityChange === 50 &&
    initialMovements[0].newStock === 50,
    'Ledger',
    'Product Opening Stock Ledger Entry Recorded (+50)'
  );
  ...
  assert(
    editedProd.sellingPrice === 165 && editedProd.stockQuantity === 50,
    'CRUD',
    'Update Product Metadata Preserves Physical Stock'
  );
  ...
  const adjDamage = await stockService.adjustStock(
    TENANT_A,
    product.id,
    {
  ...
  const adjIncrease = await stockService.adjustStock(
    TENANT_A,
    product.id,
    {
  ...
  const session = await stocktakeService.createSession(
    ...
    warehouse.id,
    ACTOR_USER,
    'Quarterly physical cycle count'
  );
  ...
  assert(
    finalProductMoong?.stockQuantity === 70,
    'Reconciliation',
    `Final On-Hand Stock = 70 (Actual: ${finalProductMoong?.stockQuantity})`
  );
  assert(
    calculatedSum === 70,
    'Reconciliation',
    `Sum of All Immutable Ledger Deltas = 70 (Actual: ${calculatedSum})`
  );
  assert(
    finalProductMoong?.stockQuantity === calculatedSum,
    'Reconciliation',
    'PERFECT MATHEMATICAL RECONCILIATION: Stock Balance Equals Ledger Sum'
  );
  ...
  assert(
    suggestions.some(s => s.productId === product.id && s.suggestedQuantity === 50),
    'Reorder',
    'Replenishment Radar Triggers Purchase Suggestion for Depleted SKU'
  );

  // Forecasting Report
  const forecast = await forecastingService.getForecastingReport(TENANT_A);
  assert(
    forecast.summary.totalSkus > 0 &&
    forecast.demandVelocity.length > 0 &&
    !isNaN(forecast.summary.totalCapitalLocked),
    'Forecasting',
    'Demand Forecasting & Dead Capital Generated without NaN Errors'
  );
...
Tata Salt 1kg,TATA-SLT-1K,8901030388902,Staples,Tata,24,28,30,100,kg,20`;
  ...
  assert(
    purchasesAfterArchive.some(p => p.items.some(it => it.productId === product.id)),
    'ArchiveSafety',
    'Historical Purchase Invoice Records Preserved after Product Archival'
  );
  ...
  try {
    mockStore.adjustStock(
      TENANT_A,
      product.id,
      { adjustmentType: 'increase', quantity: 10, reason: 'Illegal super admin edit', userId: 'sa', userName: 'Admin' },
      'platform'
    );
  } catch (err) {
    if (err instanceof AccessBoundaryViolationError) {
