...
async function run() {
    ...
    const { kot: kot1, table: orderedTable } = mockStore.fireKot(TENANT_ID, tableT02.id, [
        {
    ...
    const { kot: kot2 } = mockStore.fireKot(TENANT_ID, tableT02.id, [
        {
            menuItemId: paneerButterMasala.id,
            name: paneerButterMasala.name,
            quantity: 1,
    ...
    const { kot: voidedKot, wasteLog } = mockStore.voidKotItem(TENANT_ID, kot2.id, voidTargetItem.id, 'Captain ordered duplicate by accident', '1234', 'Manager Vikram');
    ...
    const transferAudit = mockStore.transferTable(TENANT_ID, tableT02.id, tableT06.id, 'AC too cold near door, moved to indoor booth', '1234', 'Rajesh');
    ...
    const { order: settledOrder, table: settledTable } = mockStore.settleTableBill(TENANT_ID, tableT06.id, {
        payments: [
...
//# sourceMappingURL=verify-restaurant-flow.js.map